    ...
    <div class="section">
        ...
        <div class="financial-summary">
            <div class="financial-row">
                ...
                <span
                    class="financial-value text-danger">{{ number_format($employee->total_cost, 2) . ' ' . __('SAR') }}</span>
            </div>
            <div class="financial-row">
                <span class="financial-label arabic-text">إجمالي السعر / Total Price:</span>
                <span
                    class="financial-value text-success">{{ number_format($employee->total_price, 2) . ' ' . __('SAR') }}</span>
            </div>
            <div class="financial-row">
            ...
            <div class="financial-row">
                ...
                <span
                    class="financial-value text-info">{{ number_format($employee->total_paid, 2) . ' ' . __('SAR') }}</span>
            </div>
            {{--        <div class="financial-row"> --}}
            ...
            @foreach ($employee->employeeStages->sortBy('stage.order') as $employeeStage)
                <div class="stage-details">
                    ...
                    <div class="stage-header arabic-text">
                        ...
                        <span
                            class="badge {{ $employeeStage->status == 'completed' ? 'badge-success' : 'badge-warning' }}">
                        ...
                        <tr>
                            ...
                            <td class="value">
                                @if ($employeeStage->completed_at && $employeeStage->created_at)
                                    {{ round($employeeStage->created_at->diffInDays($employeeStage->completed_at)) }}
                                    أيام / days
                                @else
                                    {{ __('N/A') }}
                                @endif
                    ...
                    <table class="info-table">
                        <tr>
                            ...
                            <td class="value text-success">
                                {{ number_format($employeeStage->price_amount, 2) . ' ' . __('SAR') }}</td>
                        </tr>
                        <tr>
                            <td class="label arabic-text">تكلفة المرحلة / Stage Cost</td>
                            <td class="value text-danger">
                                {{ number_format($employeeStage->amount_cost, 2) . ' ' . __('SAR') }}</td>
                        </tr>
                        <tr>
                    ...
                    @if ($employeeStage->walletTransaction)
                        <div style="margin-top: 8px;">
                            <strong class="arabic-text">تفاصيل الدفع / Payment Details:</strong>
                            <table class="payments-table">
                                <thead>
                                    <tr>
                                        <th class="arabic-text">المبلغ / Amount</th>
                                        <th class="arabic-text">البوابة / Gateway</th>
                                ...
                                <tbody>
                                    @php
                                        $transaction = $employeeStage->walletTransaction;
                                    @endphp
                                    <tr class="payment-status-{{ $transaction->status }}">
                                        <td>{{ number_format($transaction->amount, 2) . ' ' . __('SAR') }}</td>
                                        <td>{{ $transaction->gateway ?? 'N/A' }}</td>
                                        <td>
                                            @if ($transaction->status == 'completed')
                                                <span class="badge badge-success">مكتمل / Completed</span>
                                        ...
                                        <td>
                                            @if ($transaction->completed_at)
                                                {{ \Carbon\Carbon::parse($transaction->completed_at)->format('d M, Y') }}
                                            @else
                                                {{ __('N/A') }}
                                            @endif
                                        </td>
                    ...
                    @elseif($employeeStage->status == 'completed' && $employeeStage->wallet_transaction_id)
                        <div class="notes arabic-text" style="background-color: #fff3cd;">
                            معرف المعاملة: {{ $employeeStage->wallet_transaction_id }} - لم يتم تحميل تفاصيل المعاملة
                            <br>
                            Transaction ID: {{ $employeeStage->wallet_transaction_id }} - Transaction details not
                            loaded
                        </div>
                    @else
